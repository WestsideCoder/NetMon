...
function SiteNode({ site, onSelect, selectedId }: NodeProps) {
    ...
    <div>
      ...
      {open && site.children.map((c) => (
        <div key={c.id} className="ml-4 border-l dark:border-gray-700 pl-1">
          <SiteNode site={c} onSelect={onSelect} selectedId={selectedId} />
        </div>
      ))}
    </div>
  );
...
export default function SiteTreeView({ onSelect, selectedId = null }: Props) {
  const [tree, setTree] = useState<SiteTree[]>([]);
  useEffect(() => {
    api.get('/api/sites/tree').then((r) => setTree(r.data)).catch(() => {});
  }, []);

  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
      <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Site Hierarchy</h3>
      {tree.map((s) => <SiteNode key={s.id} site={s} onSelect={onSelect} selectedId={selectedId} />)}
